...
namespace CSharpMinds.Hellstorm
{
    class EnemyLogic : Component, IUpdatable
    {
        ...
        void Collder_Hit(ColliderComponent comp)
        {
            ...
            SceneManager.AddGameObjectToScene(GameObjectFactory.Build(new List<Interfaces.IComponent>() {
                new TransformComponent(new Common.Vector(position.X, position.Y - 20)),
                new LaserHitLogic(),
                new SpriteRenderComponent("Hellstorm\\Resources\\laserBlue08.png")
            }));
            _health--;
            if (_health < 1)
            {
        ...
        public void Update(Common.GameTime gameTime)
        {
                ...
                _playerTransform.Position.Y < _transform.Position.Y + 80)
            {
                ...
                SceneManager.AddGameObjectToScene(GameObjectFactory.Build("enemylaser", new List<Interfaces.IComponent>() {
                    new TransformComponent(new Common.Vector(_transform.Position.X-50, _transform.Position.Y + 40)),
                    new PhysicsComponent(),
                    new SpriteRenderComponent("Hellstorm\\Resources\\laserRed01.png"),
